...
=== HARD RULES ===
1. Read the candidate's EXACT last message before writing anything. Respond ONLY to what they actually said.
2. A conditional statement like "I'll accept if I get X" is NOT an acceptance — it's a counter-offer. Treat it as such.
3. "Yes" in response to a question like "do you have a competing offer?" is NOT accepting the deal. Read context carefully.
4. If the candidate is rude or uses profanity, respond professionally but firmly. Do NOT reward it.
5. NEVER repeat a phrase or argument you already used in this conversation.
6. 2–4 sentences max. Sound like a real person — warm, direct, a little casual. No markdown, bullets, asterisks, or bold.
7. NEVER reveal your budget ceiling or internal market data.
8. Only mark the deal closed if the candidate says yes clearly to YOUR specific offer — not to a side question.
9. After 3–4 exchanges, move toward making a concrete final offer if you haven't already. Real negotiations wrap up."""
    ...
    def _dynamic_context(self, avg_score: float) -> str:
        ...
        if self.deal_state == "converging" and not self.best_offer_made:
            converge = (
                f"\nThis is the moment — make a warm, human best-and-final offer: "
                f"${self.adjusted_offer:,} base + ${self.signing_bonus:,} signing bonus. "
                f"Sound genuine, not robotic. End with a direct question asking if they're in."
            )
        elif self.deal_state == "converging" and self.best_offer_made:
            converge = (
                "\nYou've already made your best offer. Hold firm but stay warm — "
                "acknowledge their position, then put the decision back on them. Don't make new promises."
            )
        elif self.deal_state == "closed":
            converge = (
                "\nDeal is closed! Be genuinely happy — sound like a real person excited to have them join. "
                "Mention the offer letter comes within 24 hours. Keep it warm and brief."
            )
    ...
    def _scripted_response(self, user_message: str, avg_score: float,
        ...
        if self.deal_state == "closed":
            if intent == "profanity":
                return "Let's keep it professional — we've got a deal and I'll have everything over to you shortly."
            if intent in ("salary_push", "salary_anchor"):
                return random.choice([
                    "Ha — we shook on it! I can't reopen it now, the comp team would have my head. "
                    "Offer letter is on its way within 24 hours.",
                    "We've already locked this in — I wouldn't be able to go back at this stage. "
                    "You'll have the formal paperwork soon. Really glad we got here.",
                ])
            if intent == "off_topic":
                return random.choice([
                    "We're all set — I'll get the paperwork moving. Feel free to reach out if anything comes up before then.",
                    "Done deal! I'll loop in HR today and you'll have the offer letter within 24 hours. Exciting stuff.",
                ])
            return random.choice([
                "We're confirmed — looking forward to officially welcoming you to the team. Offer letter incoming.",
                "All good on our end! HR will be in touch shortly with the formal paperwork. Really glad this worked out.",
                "You're in! Expect the offer letter within 24 hours. The team is going to be stoked.",
            ])
        ...
        if self.deal_state == "converging" and intent not in ("acceptance", "profanity"):
            if not self.best_offer_made:
                self.best_offer_made = True
                return random.choice([
                    f"Okay, I'll be straight with you — I went back to the team and pushed hard. "
                    f"The best I can get approved is ${self.adjusted_offer:,} base plus a ${self.signing_bonus:,} signing bonus. "
                    f"That's real movement from where we started, and honestly it's our ceiling. Does that work for you?",

                    f"I've had a conversation internally and here's where I landed: "
                    f"${self.adjusted_offer:,} base with a ${self.signing_bonus:,} signing bonus on day one. "
                    f"I think that's a strong package — what do you say?",

                    f"Look, I'll cut to it — I fought for you on this. "
                    f"${self.adjusted_offer:,} base and we'll add a ${self.signing_bonus:,} signing bonus. "
                    f"That's the best I can do and I'm being completely transparent with you. Are you in?",
                ])
            else:
                return random.choice([
                    f"I hear you, and I wish I had more room — but I genuinely don't. "
                    f"${self.adjusted_offer:,} base plus ${self.signing_bonus:,} signing is our final. "
                    f"At this point it's really your call — what's it going to be?",

                    f"I've already put everything I had on the table: ${self.adjusted_offer:,} base and ${self.signing_bonus:,} signing. "
                    f"I can't move beyond that — I'd be making promises I can't keep. "
                    f"So what's your thinking?",
                ])

        # ── True acceptance ───────────────────────────────────────────────────
        if intent == "acceptance":
            # Don't close if no real offer movement happened and score is low
            if not self.best_offer_made and avg_score < 5:
                return (
                    f"I'm glad you're interested! Just to make sure we're on the same page — "
                    f"you're saying yes to the ${offer:,} base with our full benefits package. Is that right?"
                )
            self.deal_state = "closed"
            return random.choice([
                f"That's awesome — genuinely, welcome aboard. "
                f"I'll get the updated offer letter over to you within 24 hours. "
                f"Really looking forward to having you on the team.",

                f"Love it. Honestly, this is the part of my job I enjoy most — "
                f"getting great people on board. Offer letter will be in your inbox by tomorrow. Welcome!",

                f"Perfect. I'm really happy we got there — you negotiated well and you should feel good about this. "
                f"I'll have everything drafted and sent today. The team can't wait.",

                f"Yes! Really glad we could make this work. "
                f"I'll loop in HR to get the paperwork rolling — expect the formal offer within 24 hours. Welcome to the team.",
            ])
