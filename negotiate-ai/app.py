...
# ═══════════════════════════════════════════════
    ...
    # ── Build strictly from what the user entered ──────────────────────────────
    base_sal      = profile["offer"]
    signing_bonus = profile.get("bonus", 0)
    relocation_val = profile.get("relocation", 0)

    # Equity: only year-1 vest if user entered RSU grant
    rsu_entered = profile.get("rsu_grant", 0)
    equity_year1 = round(rsu_entered / 4) if rsu_entered > 0 else 0

    # Annual bonus: only if user entered a target %
    annual_bonus_pct = profile.get("annual_bonus_pct", 0)
    annual_bonus_est = profile.get("annual_bonus_est", 0) if annual_bonus_pct > 0 else 0
    bonus_label = f"Annual Bonus ({annual_bonus_pct}% target)" if annual_bonus_pct > 0 else "Annual Bonus"

    # Benefits: ONLY explicitly entered stipends (no tier estimates)
    ...
    meal_annual  = profile.get("meal_stipend", 0) * 12
    benefits_entered = prof_dev + home_office + wellness + meal_annual

    # Refresher rate for equity projections below (default 18% if not entered)
    refresher_pct = profile.get("annual_refresher_pct", 0) / 100 if profile.get("annual_refresher_pct", 0) > 0 else 0.18

    total_first_year = (
        base_sal + equity_year1 + annual_bonus_est
        + signing_bonus + relocation_val + benefits_entered
    )

    # Subtitle dynamically reflects what's included
    _included = ["base salary"]
    if equity_year1 > 0:        _included.append("equity")
    if annual_bonus_est > 0:    _included.append("bonus")
    if signing_bonus > 0:       _included.append("sign-on")
    if relocation_val > 0:      _included.append("relocation")
    if benefits_entered > 0:    _included.append("stipends")
    _subtitle = "Based on values you entered: " + ", ".join(_included) + "."
    if len(_included) == 1:
        _subtitle += " Add equity, bonus, or benefits in the profile for a fuller picture."

    st.markdown(
        f'<p style="color:#64748B;font-size:0.85rem;margin-bottom:8px;">{_subtitle}</p>',
        unsafe_allow_html=True,
    )
    ...
    # ── Donut chart ────────────────────────────────────────────────────────────
    with col_donut:
        d_labels = ["Base Salary", f"Equity (Yr 1 of 4)", bonus_label,
                    "Sign-On Bonus", "Relocation", "Stipends & Perks"]
        d_values = [base_sal, equity_year1, annual_bonus_est,
                    signing_bonus, relocation_val, benefits_entered]
        d_colors = ["#10B981", "#3B82F6", "#F59E0B", "#EC4899", "#F97316", "#A78BFA"]
        filtered = [(l, v, c) for l, v, c in zip(d_labels, d_values, d_colors) if v > 0]
        fl, fv, fc = zip(*filtered) if filtered else (["Base Salary"], [base_sal], ["#10B981"])
    ...
    with col_breakdown:
        comp_items = [
            ...
            ("Equity (Yr 1 of 4)", equity_year1, "#3B82F6"),
            (bonus_label, annual_bonus_est, "#F59E0B"),
            ("Sign-On Bonus", signing_bonus, "#EC4899"),
            ("Relocation", relocation_val, "#F97316"),
            ("Stipends & Perks", benefits_entered, "#A78BFA"),
        ]
        bar_labels = [lbl for lbl, val, _ in comp_items if val > 0]
        bar_values = [val for _, val, _ in comp_items if val > 0]
        bar_colors = [clr for _, val, clr in comp_items if val > 0]

        # Auto-scale text: inside bars for large values, outside for small
        max_val = max(bar_values) if bar_values else 1
        text_positions = [
            "inside" if v / max_val > 0.35 else "outside"
            for v in bar_values
        ]

        fig_bar = go.Figure()
        for lbl, val, clr, pos in zip(bar_labels, bar_values, bar_colors, text_positions):
            fig_bar.add_trace(go.Bar(
                ...
                textposition=pos,
                textfont=dict(
                    family="DM Mono, monospace", size=11,
                    color="#022C22" if pos == "inside" else "#CBD5E1",
                ),
                hovertemplate=f"{lbl}: ${{x:,.0f}}<extra></extra>",
                showlegend=False,
            ))

        fig_bar.update_layout(
            height=max(220, len(bar_labels) * 52),
            ...
            bargap=0.3,
            annotations=[dict(
                text=f"<b>Total: ${total_first_year:,}</b>",
                xref="paper", yref="paper",
                x=0.5, y=1.07, xanchor="center",
        ...
        if len(_included) == 1:
            st.caption("Tip: open the Equity, Benefits & Perks sections in the profile to see your full comp picture.")
    ...
    if rsu_entered == 0:
        st.markdown(
            ...
            'No RSU grant entered — open the <b style="color:#94A3B8;">Equity & Long-term Incentives</b> '
            'section in the profile to model your equity vesting over 4 years.</p>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
    ...
    if rsu_entered > 0:
        ...
        init_per_yr = rsu_entered / 4
        refresh_grant = rsu_entered * refresher_pct
        ...
        <p style="font-family:DM Sans,sans-serif;font-size:0.68rem;color:#475569;margin:0 0 20px 0;">
            ℹ Refreshers modeled at {round(refresher_pct*100)}% of initial grant/year {"(entered)" if profile.get("annual_refresher_pct",0) > 0 else "(default 18% — enter your refresher rate in the profile for precision)"}. Based on your entered RSU grant of ${rsu_entered:,}.
